#!/usr/bin/env python3
"""
Experiment 4: Cross-Series Model Evaluation
Evaluate knowledge extraction across different model families at similar parameter counts.
"""
import asyncio
import json
import os
...
FUZZY_CACHE_PATH = "results/cross_series_fuzzy_cache.json"
...
async def process_model_to_unique(model, domain_id, processor, judge, fuzzy_cache, audit_cache, pbar_pos, lock):
    """Process a single model's raw points through 3-stage deduplication and audit."""
    folder_name = model.split("/")[-1]
    m_dir = os.path.join(RESULTS_DIR, domain_id, folder_name)

    if not os.path.exists(m_dir):
        print(f"      [Warning] Directory not found: {m_dir}", flush=True)
        return []

    raw_file = os.path.join(m_dir, "P4_Taxonomy_L5W5_raw.json")
    if not os.path.exists(raw_file):
        print(f"      [Warning] Raw file not found: {raw_file}", flush=True)
        return []

    print(f"      📄 Loading raw file...", flush=True)
    with open(raw_file, "r") as f:
        points = json.load(f)
    if not points:
        return []
    print(f"      📊 Loaded {len(points)} raw points", flush=True)

    # Stage 1: Strict Deduplication (≥0.92)
    print(f"      🔍 Stage 1: Getting embeddings...", flush=True)
    embs = await processor.get_embeddings(points)
    print(f"      🔍 Stage 1: Computing strict dedup (≥0.92)...", flush=True)
    norms = np.array(embs)
    ...
    print(f"      ✓ Stage 1: {len(remaining_indices)} points after strict dedup", flush=True)

    # Stage 2: Fuzzy Deduplication (0.70-0.92)
    print(f"      🔍 Stage 2: Fuzzy dedup (0.70-0.92)...", flush=True)
    unique_indices = []
    fuzzy_tasks = []
    ...
    print(f"      ✓ Cache check: {len(fuzzy_tasks)} need LLM, {len(unique_indices)} resolved", flush=True)

    if fuzzy_tasks:
        print(f"      🔍 Stage 2: {len(fuzzy_tasks)} pairs need LLM judge...", flush=True)
        pbar = tqdm(total=len(fuzzy_tasks), desc=f"      Fuzzy [{model.split('/')[-1][:8]}]", position=pbar_pos, leave=False)
    ...
    print(f"      ✓ Stage 2: {len(unique_indices)} points after fuzzy dedup", flush=True)

    # Stage 3: Domain Audit
    print(f"      🔍 Stage 3: Domain audit...", flush=True)
    final_indices = []
    audit_needed = []
    ...
    for idx in unique_indices:
        ...
        if res is True:
            ...
            audit_needed.append(idx)

    if audit_needed:
        print(f"      🔍 Stage 3: {len(audit_needed)} points need domain audit...", flush=True)
        pbar = tqdm(total=len(audit_needed), desc=f"      Audit [{model.split('/')[-1][:8]}]", position=pbar_pos, leave=False)
    ...
    print(f"      ✓ Stage 3: {len(final_indices)} valid points after audit", flush=True)
...
async def build_union_with_dedup(all_points, processor, fuzzy_cache, lock, pbar_pos):
    """Build union set with two-stage deduplication."""
    if not all_points:
        return []
...
async def compute_unique_discovery(model_points, other_models_points, processor, lock):
    """Compute unique discovery using semantic matching."""
...
async def process_domain_task(domain_id, domain_query, models, processor, judge, fuzzy_cache, audit_cache, lock):
    """Process a single domain with all models."""
    print(f"\n{'='*80}", flush=True)
    ...
    if not all_valid_points:
        print(f"   ⚠️  No valid points found for {domain_id}", flush=True)
    ...
    print(f"      ✓ Union: {len(union_texts)} unique points", flush=True)
    ...
    for model in models:
        ...
        print(f"      {model_name}: Recall={recall:.2%}, Accuracy={accuracy:.2%}, Unique Discovery={unique_disc}", flush=True)
...
async def main_evaluation():
    print("\n" + "=" * 80, flush=True)
    print("🚀 Experiment 4: Cross-Series Model Evaluation (~7B)", flush=True)
    print("=" * 80, flush=True)
    ...
    print("🔄 Starting sequential domain processing...", flush=True)
    results = []
    for d_id, d_query in DOMAINS.items():
        print(f"\n   • Processing domain: {d_id}", flush=True)
        result = await process_domain_task(d_id, d_query, DEFAULT_MODELS, processor, judge, fuzzy_cache, audit_cache, lock)
    ...
    summary_path = os.path.join(RESULTS_DIR, "cross_series_summary.txt")
    with open(summary_path, "w") as f:
        f.write("="*120 + "\n")
        f.write("Cross-Series 7B Model Comparison Summary\n")
        f.write("="*120 + "\n\n")
    ...
    print("=" * 80, flush=True)
    print(json.dumps(dict(results), indent=2), flush=True)
    print("=" * 80 + "\n", flush=True)
    print(f"📄 JSON Report saved to: {report_path}", flush=True)
    print(f"📄 Summary Table saved to: {summary_path}", flush=True)
