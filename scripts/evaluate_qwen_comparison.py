...
FUZZY_CACHE_PATH = "results/evolution_fuzzy_cache.json"
...
async def process_model_to_unique(model, domain_id, processor, judge, fuzzy_cache, audit_cache, pbar_pos, lock):
    # Match the folder naming from run_evolution_comparison.py
    folder_name = model.replace("/", "_")  # "qwen/qwen2.5-7b-instruct" -> "qwen_qwen2.5-7b-instruct"
...
async def main_evaluation():
    import sys
    print("\n" + "="*80, flush=True)
    print("🚀 Qwen RL Comparison Evaluation", flush=True)
    print("="*80, flush=True)
    print(f"📂 Results Directory: {RESULTS_DIR}", flush=True)
    print(f"📊 Domains: {list(DOMAINS.keys())}", flush=True)
    print(f"🤖 Models: {[m.split('/')[-1] for m in MODELS]}", flush=True)
    ...
    for d_id, d_query in DOMAINS.items():
        ...
        result = await process_domain_task(d_id, d_query, MODELS, processor, judge, fuzzy_cache, audit_cache, 0, lock)
    ...
    summary_path = os.path.join(RESULTS_DIR, "qwen_comparison_summary.txt")
    with open(summary_path, "w") as f:
        f.write("="*120 + "\n")
        f.write("Qwen RL Comparison Evaluation Summary\n")
        f.write("="*120 + "\n\n")
