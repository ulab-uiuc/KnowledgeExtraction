#!/usr/bin/env python3
"""
Plot knowledge growth curves for Qwen RL comparison experiment.
...
# --- Configuration ---
RESULTS_DIR = "results/evolution_comparison"
REPORT_PATH = os.path.join(RESULTS_DIR, "qwen_comparison_report.json")
AUDIT_CACHE_PATH = "results/evolution_audit_cache.json"
PLOTS_DIR = os.path.join(RESULTS_DIR, "plots")
GROWTH_DATA_PATH = os.path.join(RESULTS_DIR, "growth_data.json")
COMBINED_OUTPUT = "paper/img/rl_curve.pdf"
...
MODEL_DISPLAY_NAMES = {
    "qwen/qwen2.5-7b-instruct": "Qwen 2.5 7B (General)",
    "qwen/qwen2.5-coder-7b-instruct": "Qwen 2.5 7B Coder (RL-tuned)"
}

MODEL_COLORS = {
    "qwen/qwen2.5-7b-instruct": "#ff7f0e",    # Orange
    "qwen/qwen2.5-coder-7b-instruct": "#2ca02c"  # Green
}
...
def plot_combined_grid(all_growth_data, output_path):
    ...
    # Double column ICML figure size (recommended: 7.0 x 3.0 for flat aspect ratio)
...
def main():
    print("\n" + "=" * 80)
    print("📈 Plotting Qwen RL Comparison Knowledge Growth Curves")
    print("=" * 80)
    ...
    if not os.path.exists(REPORT_PATH):
        ...
        print("   Please run evaluate_qwen_comparison.py first!")
        return
    ...
    for domain_id in DOMAINS.keys():
        ...
        for model in MODELS:
            # Match folder naming: "qwen/qwen2.5-7b-instruct" -> "qwen_qwen2.5-7b-instruct"
            folder_name = model.replace("/", "_")
