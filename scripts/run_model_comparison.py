#!/usr/bin/env python3
"""
Experiment 2: Llama 3.1 Scaling Law Analysis (Size Comparison)
Compare knowledge extraction across Llama 3.1 family (8B, 70B, 405B).
Uses parallel execution like run_cross_series.py.
"""
import asyncio
import json
import os
...
OUTPUT_BASE_DIR = "results/model_comparison_l5w5"
MAX_TURNS = 15  # Full experiment

# Aligned with cross_series and RL experiment domains
TARGET_DOMAINS = {
...
async def run_extraction_task(domain_id, domain_query, model_name, api_keys, processor, pbar):
    """Executes a single extraction pipeline (parallel-safe)."""
    ...
    gen_agent = GenAgent(api_key=api_keys, model=model_name, base_url="https://integrate.api.nvidia.com/v1")
    ...
    try:
        ...
        try:
            ...
            with open("model_comparison_errors.log", "a") as err_f:
...
async def main():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 Experiment 2: Llama 3.1 Scaling Law Analysis")
    print(f"{'='*60}")
    print(f"📂 Output: {OUTPUT_BASE_DIR}")
    print(f"🤖 Models: {[m.split('/')[-1] for m in SCALING_MODELS]}")
